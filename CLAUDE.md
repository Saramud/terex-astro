...
- `public/llms.txt` — hand-maintained markdown index served at `/llms.txt` (the [llms.txt](https://llmstxt.org) convention): site summary, standard rental terms, all 14 categories with min prices + links, articles, and contact pages. It's a content guide for LLMs, **not** a crawler-directive file. Keep category prices in sync with `equipment.ts` minimums. Note: `public/robots.txt` already allows AI crawlers via `User-agent: *` (only `/mail.php` and `/telegram.php` are disallowed) — intentional, so the site can surface in AI answers.
